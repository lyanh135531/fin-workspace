...
readonly sql_file="${script_dir}/financial-plan/monitor-operational-health.sql"
readonly timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
readonly report_dir="${1:-${project_dir}/backups/financial-plan-audit}"
readonly report_file="${report_dir}/health_${timestamp}.json"
...
node -e '
  ...
  const report = JSON.parse(fs.readFileSync(process.argv[1], "utf8").trim());
  fs.writeFileSync(process.argv[2], `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
  console.log(`Financial plan health report: ${process.argv[2]}`);
  console.log(`Issues: ${report.issueCount}; active plans: ${report.activePlans}`);
