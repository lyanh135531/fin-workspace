...
readonly sql_file="${script_dir}/financial-plan/audit-contract-cleanup.sql"
readonly timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
readonly report_dir="${1:-${project_dir}/backups/financial-plan-audit}"
readonly report_file="${report_dir}/contract_audit_${timestamp}.json"
readonly partial_file="${report_file}.partial"

cleanup() { rm -f -- "${partial_file}"; }
compose() {
  docker compose --project-directory "${project_dir}" --file "${project_dir}/docker-compose.yml" "$@"
}
...
node -e '
  ...
  console.log(`Contract cleanup audit: ${target}`);
  console.log(`Templates: ${report.userTemplates}; globals: ${report.globalCategories}; safe: ${report.safeToContract}`);
  if (!report.safeToContract) process.exitCode = 2;
