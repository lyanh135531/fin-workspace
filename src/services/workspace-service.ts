...
import { DEFAULT_WORKSPACE_CATEGORIES } from "@/domain";
...
async function createWorkspaceInTransaction(
  ...
  await tx.category.createMany({
    data: DEFAULT_WORKSPACE_CATEGORIES.map((category) => ({
      workspaceId: workspace.id,
      name: category.name,
      code: category.code,
  ...
  await tx.auditLog.create({ data: { workspaceId: workspace.id, actorUserId: userId, action: "workspace.created", entityType: "workspace", entityId: workspace.id, metadata: { creatorRole: "ADMIN", defaultCategoryCount: DEFAULT_WORKSPACE_CATEGORIES.length } } });
  return workspace;
}
...
export async function createInitialWorkspaceForUser(
  ...
  throw new AppError("CONFLICT", "Không thể tạo nhóm tài chính. Vui lòng thử lại.");
}
...
export async function deleteWorkspaceForUser(userId: string, workspaceId: string) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (!workspace || workspace.deletedAt) throw new AppError("NOT_FOUND", "Không tìm thấy nhóm tài chính.");
