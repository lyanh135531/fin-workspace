...
export async function requestJoinAction(input:unknown){try{const s=await getServerSession(authOptions);if(!s?.user?.id)throw new AppError("AUTHENTICATION_REQUIRED","Please sign in.");await requestWorkspaceJoin(s.user.id,z.object({inviteCode:z.string().trim().min(6).max(36)}).parse(input).inviteCode);revalidatePath("/dashboard/join");return {ok:true,message:null};}catch(e){return fail(e);}}
