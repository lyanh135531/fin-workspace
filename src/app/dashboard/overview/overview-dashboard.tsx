...
type Transaction = { id: string; amount: string; type: "income" | "expense" | "transfer" | "investment_buy" | "investment_sell"; status: "pending" | "scheduled" | "approved" | "rejected"; description: string | null; date: string; walletId: string; toWalletId: string | null; wallet: string; categoryId: string | null; category: { name: string; color: string } | null; memberId: string; member: string };
    ...
    <div className="overview-grid"><CashflowOverviewCharts members={members} transactions={transactions} currency={workspace.currency} month={reportPeriod} range={range} onRangeChange={setRange} walletId={walletId} categoryId={categoryId} memberId={memberId} transactionType={type} categoryType={categories.find((category) => category.id === categoryId)?.type}/>
      ...
      <div className="overview-detail-grid">
        ...
        <Card as="section" className="overview-card overview-recent gap-0 py-0">
          ...
          <div className="recent-table">{filtered.slice(0, 6).map((item) => <article key={item.id}><div><strong title={item.description ?? "Không có nội dung"}>{item.description ?? "Không có nội dung"}</strong><small>{item.category?.name ?? "Chưa phân loại"} · {item.wallet} · {item.member}</small></div><time>{new Intl.DateTimeFormat("vi-VN", { day: "2-digit", month: "2-digit" }).format(new Date(item.date))}</time><b className={item.type === "investment_buy" ? "expense" : item.type === "investment_sell" ? "income" : item.type}>{item.type === "income" || item.type === "investment_sell" ? "+" : item.type === "expense" || item.type === "investment_buy" ? "−" : "↔"}{money(item.amount, workspace.currency)}</b><span className={`overview-status ${item.status}`}>{statusLabel[item.status]}</span></article>)}{!filtered.length && <Empty variant="compact" title="Không có giao dịch phù hợp" description="Thử thay đổi bộ lọc báo cáo." />}</div>
        </Card>
