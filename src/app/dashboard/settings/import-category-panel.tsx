...
import { ArrowDownLeft, ArrowUpRight, Download, Tag } from "lucide-react";
  ...
  if (templates.length === 0) {
    return (
      <section className="sunrise-card mt-4 p-6">
        ...
        <h2 className="mt-1 text-xl font-semibold">Import từ danh mục mẫu</h2>
        <div className="mt-4 p-6 text-center border border-dashed border-[var(--border)] rounded-xl">
  ...
  return (
    <section className="sunrise-card mt-4 p-6">
        ...
        <div className="mt-4 space-y-2">
          {importableTemplates.map((t) => {
            ...
            return (
              <label
                key={t.id}
                className={`flex min-h-12 items-center gap-3 rounded-lg border px-3 py-2 cursor-pointer transition-all ${
                  isChecked
      ...
      {alreadyImported.length > 0 && (
        <div className="mt-4">
          <p className="text-xs font-semibold text-slate-400 mb-2">Đã có trong workspace (bỏ qua)</p>
