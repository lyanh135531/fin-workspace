...
export async function changePasswordAction(input: unknown) { try { const data = z.object({ currentPassword: z.string().min(1, "Vui lòng nhập mật khẩu hiện tại."), newPassword: z.string().min(6, "Mật khẩu mới phải có tối thiểu 6 ký tự.").max(128) }).parse(input); await changeOwnPassword(await actor(), data.currentPassword, data.newPassword); return { ok: true, message: null }; } catch (error) { return result(error); } }
